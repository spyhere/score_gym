 ...
 export function generatingValues() {
    ...
    while (bars.length < barsQuant) {
        ...
        if (fill === measure) {
            ...
            let beamGr = 0;
            let beamGrNames = [];
            bar.forEach((x, ind) => {           // BEAMING EIGHTH, SIXTEENTH AND THIRTY-SECOND
                beamFill += x[1];
                ...
                if ((/rest/).test(x[0])) {      // GROUPING RESTS
                    let a = null;
                    if (ind+1 <= bar.length-1 && (/rest/).test(bar[ind+1][0]) && groupRest) {
                        
                        values.forEach((z) => z.forEach((y) => {
                            if (y[2] === x[1]+bar[ind+1][1] && !a) {
                                console.log(x[0])
                                bar[ind][0] = y[1];
                                bar[ind][1] = y[2];
                                a = true;
                    ...
                    if (ind+1 <= bar.length-1 && (names4Beaming[0].test(bar[ind+1][0]) && !names4Beaming[1].test(bar[ind+1][0]))) {
                        // console.log(beamGr + "   " + x[0])
                                        if (beamGr === x[1] && beamFill !== measure/2 && (ind === 0 || !(/--st|--gr/).test(bar[ind-1][0])) &&
                                         (ind === 0 || !((/rest/).test(bar[ind-1][0]) && Number.isInteger(beamFill))) ) {

                                            if ((/eighth/).test(x[0])) {                           // START OF THE GROUP
                                                bar[ind][0] = bar[ind][0] + "--st";
                                            }
                                            else if ((new RegExp((x[0].match(/\D/g)).join(""))).test(bar[ind+1][0])) {
                                                bar[ind][0] = bar[ind][0] + "--1--st";
                                            }
                                            else {
                                                bar[ind][0] = bar[ind][0] + "--0--st";
                                            }
                                            
                                        }
                                        else if (ind>0 && (/--st|--gr/).test(bar[ind-1][0]) && beamFill !== measure/2 && beamGr !== beamNum 
                                        ) {                                 
                                             
                                            if ((/eighth/).test(x[0])) {                           // MIDDLE SECTION
                                                bar[ind][0] = bar[ind][0] + "--gr";
                                                }
                                                else if (!(new RegExp((x[0].match(/\D/g)).join(""))).test(bar[ind+1][0]) && 
                                                !(new RegExp((x[0].match(/\D/g)).join(""))).test(bar[ind-1][0])) {
                                                    bar[ind][0] = bar[ind][0] + "--2--gr";
                                                }
                                                else if ((new RegExp((x[0].match(/\D/g)).join(""))).test(bar[ind+1][0])) {
                                                    bar[ind][0] = bar[ind][0] + "--1--gr";
                                                }
                                                else {
                                                    bar[ind][0] = bar[ind][0] + "--0--gr";
                                                } 
                                    ...
                                    if (!(/--st|--gr/).test(x[0]) && ind>0 && (/--st|--gr/).test(bar[ind-1][0]) &&
(ind === bar.length-1 || beamGr === beamNum || beamFill === measure/2 || !names4Beaming[0].test(bar[ind+1][0]) || names4Beaming[1].test(bar[ind+1][0]))) {
                                        if ((/eighth/).test(x[0])) {                            // ENDING SECTION
                                            bar[ind][0] = bar[ind][0] + "--end";
                                        }
                                        else if ((new RegExp((x[0].match(/\D/g)).join(""))).test(bar[ind-1][0])) {
                                            bar[ind][0] = bar[ind][0] + "--1--end";
                                        }
                                        else {
                                            bar[ind][0] = bar[ind][0] + "--0--end";
                                        }
                                        beamGr = 0;
                                        beamGrNames = [];
                ...
                // console.log(x[0] + " " + beamGr + " " + beamGrNames)
            })
            // console.log("end")
            bars.push(bar);
            bar = [];
